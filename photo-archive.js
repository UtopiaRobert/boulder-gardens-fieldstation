(() => {
  ...
  viewer.innerHTML = `
    <div class="fs-gallery-viewer__stage"
      ...
      <p class="fs-gallery-viewer__caption">
        <span class="fs-gallery-viewer__caption-copy">
          <span class="fs-gallery-viewer__caption-text"></span>
          <span class="fs-gallery-viewer__caption-separator"
                aria-hidden="true">·</span>
          <time class="fs-gallery-viewer__caption-date"></time>
        </span>
  ...
  const captionDate = viewer.querySelector('.fs-gallery-viewer__caption-date');
  const captionSeparator = viewer.querySelector('.fs-gallery-viewer__caption-separator');
  ...
  function updateViewer() {
    ...
    image.src = item.src;
    image.alt =
      item.alt || description || 'Boulder Gardens Fieldstation photograph';

    caption.textContent = description;
    caption.hidden = !description;

    captionDate.textContent = displayDate;
    captionDate.hidden = !displayDate;

    if (item.dateISO) {
      captionDate.dateTime = item.dateISO;
    } else {
      captionDate.removeAttribute('datetime');
    }

    captionSeparator.hidden = !(description && displayDate);
